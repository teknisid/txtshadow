...
$(function() {
    ...
    function generateCSS(){
        ...
        for( var objVal in shadows ) {
            $rows += shadows[objVal].offsetH+'px '+shadows[objVal].offsetV+'px '+shadows[objVal].radiusB+'px rgba('+shadows[objVal].R+', '+shadows[objVal].G+', '+shadows[objVal].B+', '+shadows[objVal].opacity+'),';
        }
		
        var $vals = $rows .substring(0, $rows.length - 1);

		$('#demo').css({ 'text-shadow': $vals });
		$('#code').html( 'text-shadow: '+$vals+';');
		Prism.highlightAll();
    }
   ...
   // $(document).on('mouseenter mouseleave', '.add-s-li', function(e) {
   //     if( e.type === 'mouseenter' ){
   //         $('.showx',this).show();
   //         var $shNum = $('.showx',this).data("shnum");
   //         var $preview;
   //         $preview = shadows['shadow'+$shNum].offsetH+'px '+shadows['shadow'+$shNum].offsetV+'px '+shadows['shadow'+$shNum].radiusB+'px rgba( '+shadows['shadow'+$shNum].R+', '+shadows['shadow'+$shNum].G+', '+shadows['shadow'+$shNum].B+','+shadows['shadow'+$shNum].opacity+')';               
   //         $('#demo').css({ 'text-shadow': $preview });
   //     } else {
   //         $('.showx',this).hide();
   //         generateCSS();
   //     }
